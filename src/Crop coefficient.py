...
Crop_list = ['Alfalfa_1', 'Alfalfa_2', 'Potato', 'Sweet_Melon', 'Sugar_beets', 'Sunflower', 'Sesame', 'Rapeseed', 'Cotton', 'Barley', 'Soybeans', 'Sorghum', 'Rice', 'Maize', 'ReMaize', 'Winter_Wheat', 'Wheat']

h_list = [0.7, 0.7, 0.6, 0.4, 0.5, 2, 1, 0.6, 1.3, 1, 0.7, 3, 1, 2, 2, 1, 1]
Kcmid_table_list = [1.2, 1.2, 1.15, 1.05, 1.2, 1.15, 1.1, 1.15, 1.15, 1.15, 1.15, 1.2, 1.2, 1.2, 1.2, 1.15, 1.15]
Kcend_table_list = [1.15, 1.15, 0.75, 0.75, 0.7, 0.35, 0.25, 0.35, 0.6, 0.25, 0.5, 1.05, 0.7, 0.35, 0.5, 0.3, 0.3]
Lini_list = [20, 20, 30, 25, 25, 25, 20, 30, 30, 15, 20, 20, 30, 20, 15, 30, 20]
...
Month_Day_1_list = [6, 7, 11, 16, 25, 16, 6, 6, 3, 11, 12, 11, 15, 11, 11, 16, 25]
Month_Day_2_list = [14, 13, 19, 9, 25, 9, 14, 24, 27, 4, 8, 9, 15, 9, 4, 14, 25]
...
for year in year_list:
    for i in range(len(Crop_list)):
        ...
        if Month_1 != Month_2:
            ...
            Prec_total = (Raster(Prec_1) * Month_Day_1 / Month_All_day_1 + Raster(Prec_2) * Month_Day_2 / Month_All_day_2)  # 大多数植被生长跨月份，根据月内天数比例求生长初期的总降水量
            Wind_mean = (Raster(Wind_1) * Month_Day_1 / Lini + Raster(Wind_2) * Month_Day_2 / Lini)
            Temn_mean = (Raster(Temn_1) * Month_Day_1 / Lini + Raster(Temn_2) * Month_Day_2 / Lini)
            Temx_mean = (Raster(Temx_1) * Month_Day_1 / Lini + Raster(Temx_2) * Month_Day_2 / Lini)
            ET0 = Raster(Pett_1) / Month_All_day_1 * Month_Day_1 / Lini + Raster(Pett_2) / Month_All_day_2 * Month_Day_2 / Lini

        else:

            Prec_total = (Raster(Prec_1) * Lini / Month_All_day_1)  # 大多数植被生长跨月份，根据月内天数比例求生长初期的总降水量
        ...
        I = (Prec_total + Irrigation) / nw  # 在播种之前进行一次灌溉，(降水+灌溉)/(降水次数+灌溉次数)
        Tw = Lini / (nw + 0.5)
        ...
        REW_10 = Con((Con((6 / Power(ET0, 0.5)) > 2.5, (6 / Power(ET0, 0.5)), 2.5)) > 7, 7, (Con((6 / Power(ET0, 0.5)) > 2.5, (6 / Power(ET0, 0.5)), 2.5)))
        REW_40 = Con((TEW_40-0.01) > 6, 6, (TEW_40-0.01))
        T1_10 = REW_10 / Eso
        T1_40 = REW_40 / Eso

        Kcini_10 = Con((Con(T1_10 > Tw, 1.15, (TEW_10 - (TEW_10 - REW_10) * Exp(-(Tw - T1_10) * Eso * (1 + REW_10 / (TEW_10 - REW_10))/TEW_10))/(Tw * ET0))) < 1.15, (Con(T1_10 > Tw, 1.15, (TEW_10 - (TEW_10 - REW_10) * Exp(-(Tw - T1_10) * Eso * (1 + REW_10 / (TEW_10 - REW_10))/TEW_10))/(Tw * ET0))), 1.15)
        Kcini_40 = Con((Con(T1_40 > Tw, 1.15, (TEW_40 - (TEW_40 - REW_40) * Exp(-(Tw - T1_40) * Eso * (1 + REW_40 / (TEW_40 - REW_40))/TEW_40))/(Tw * ET0))) < 1.15, (Con(T1_40 > Tw, 1.15, (TEW_40 - (TEW_40 - REW_40) * Exp(-(Tw - T1_40) * Eso * (1 + REW_40 / (TEW_40 - REW_40))/TEW_40))/(Tw * ET0))), 1.15)
        Kcini = Con(I < 10, Kcini_10, Kcini_10 + (I - 10)/(40 - 10) * (Kcini_40 - Kcini_10))
        Kcmid = Kcmid_table + (0.04 * (Wind_mean - 2) - 0.004 * (RH_min - 45)) * Power((ET0 * 0 + h)/3, 0.3)
        Kcend = Con(Kcend_table < 0.45, ET0 * 0 + Kcend_table, Kcend_table + (0.04 * (Wind_mean - 2) - 0.004 * (RH_min - 45)) * Power((ET0 * 0 + h) / 3, 0.3))
